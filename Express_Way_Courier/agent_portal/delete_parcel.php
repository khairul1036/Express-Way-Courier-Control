...
<img src="user_img/<?php echo "$image";?>" alt="Avatar" class="avatar"> 
  ...
  <a href="delete_parcel.php"  class="active"><i class="fa fa-trash" aria-hidden="true"></i>Delete Parcel</a>
  </div>
</div>
<!-- sidebar -->
...
<!-- header -->
    ...
    <div class="container">
    <div>
       <p class="text">Delete Parcel List</p>
    </div>
    <div class="horizontal-line"></div>
    <?php
    if(isset($_SESSION['contact'])){
    ...
    $rs_result=mysqli_query($con,"Select * from delete_parcels where from_branch_id='$id'");
    ...
    $sql="Select * from delete_parcels where from_branch_id='$id' limit ".$startinglimit.','.$numberPages;
    ...
    <table class="table1">
      ...
      <?php
          for ($i = 1; $i <= $totalPages; $i++) {
              $activeClass = ($i == $currentPage) ? 'delete-active-btn' : '';
              echo '<a class="delete-pagination-btn ' . $activeClass . '" href="delete_parcel.php?page=' . $i . '">' . $i . '</a>';
          }
    ?>
    </div>
